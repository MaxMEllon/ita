...
const tryGetToken = async state => {
  ...
  if (!auth)
    throw new Error("You must sign in or sign up before show the todo list");
  return auth.token;
};
...
const trySetConfig = async state => {
  ...
  if (!config)
    throw new Error(
      "You must create ~/.config/ita/init.json before show the todo list"
    );
  return config;
};
      ...
      attachments: [
        {
          ...
          fields: list.map(todo => ({
            title: `:white_check_mark: ${todo.percent.padStart(
              5
            )} ${todo.title}`
          }))
        }
